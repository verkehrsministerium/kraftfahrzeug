...
use cursive::event::{AnyCb, Event, EventResult, EventTrigger};
use cursive::utils::markup::StyledString;
use cursive::view::{Identifiable, Selector, View};
use cursive::views::{IdView, OnEventView, SelectView};
use cursive::{vec::Vec2, Cursive, Printer, Rect};
use std::cell::Cell;
use std::rc::Rc;

use super::{ExpansionState, Value, ValueKind};
use crate::utils;
...
impl MessageView {
    ...
    pub fn new(message: Rc<Value<'static>>) -> Self {
        ...
        list.set_on_submit(
            move |siv: &mut Cursive, expansion_state: &Rc<Cell<ExpansionState>>| {
                match expansion_state.get() {
                    ExpansionState::Collapsed => {
                        log::debug!("expanding object");
                        expansion_state.set(ExpansionState::Expanded)
                    }
                    ExpansionState::Expanded => {
                        log::debug!("collapsing object");
                        expansion_state.set(ExpansionState::Collapsed)
                    }
    ...
    fn indent(message: &Value, level: usize) -> Vec<(StyledString, Rc<Cell<ExpansionState>>)> {
